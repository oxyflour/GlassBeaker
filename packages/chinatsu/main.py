...
class MnaCircuit(Circuit):
    ...
    def node_voltages(self, power, phase) -> Tensor:
    ...
    def component_currents(self, voltages: Tensor, comp: Network):
        ...
        for port in range(comp.nports):
            i[:, port] = (v * comp.y[:, :, port]).sum(dim=1) #type: ignore
        return i
...
class Test(unittest.TestCase):
    def __init__(self, methodName: str = "runTest") -> None:
        ...
        z = TensorGammaZ0(f)
        n = z.resistor(50.0, name="r0") #type: ignore
        ...
        rs = [z.resistor(50.0, name=f'res-{i}') for i in range(3)] #type: ignore
        cx_list = [
            [
                [(p0, 0), (n, 0)],
                [(n, 1), (g, 0)],
    ...
    def test_performance(self):
        ...
        p0 = Circuit.Port(f, name='port-0')
        g = Circuit.Ground(f, name='gnd-0')
        rs = [z.resistor(50.0, name=f'res-{i}') for i in range(50)] #type: ignore
        cx = [
            [(p0, 0), (n, 0)],
            [(n, 1), (rs[0], 0)],
            *[[(ri, 1), (rj, 0)] for ri, rj in zip(rs[0:-1], rs[1:])],
    ...
    def create_circuit(self, repeat = 1):
        ...
        p0 = Circuit.Port(f, name='port-0')
        g = Circuit.Ground(f, name='gnd-0')
        r = z.resistor(50.0, name='r-0') #type: ignore
