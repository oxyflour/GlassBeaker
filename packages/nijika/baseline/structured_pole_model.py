...
class StructuredPoleResiduePredictor(nn.Module):
    def __init__(
        self,
        ...
        self.pole_head = nn.Sequential(
            nn.Linear(hidden_dim, hidden_dim),
            nn.GELU(),
            nn.Linear(hidden_dim, num_poles * 2),
        )
    ...
    def _decode_pairs(self, pair_latent: torch.Tensor, poles: torch.Tensor) -> torch.Tensor:
        ...
        residue_raw = raw[..., 4:].view(pair_latent.size(0), len(self.pairs), self.num_poles, 2)
    ...
    def forward(
        ...
        if frame is None or cuts is None or nibs is None:
            raise ValueError("StructuredPoleResiduePredictor requires frame/cuts/nibs features")
