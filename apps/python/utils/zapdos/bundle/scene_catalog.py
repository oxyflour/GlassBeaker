...
def object_root_prims(default_prim):
    for child in default_prim.GetChildren():
        if child.GetName() in TRANSPARENT_CONTAINER_NAMES and child.IsA(UsdGeom.Xformable):  # type: ignore[arg-type]
...
def scene_object_spec(default_path: PurePosixPath, prim) -> SceneObjectSpec | None:
    if not prim.IsValid() or not prim.IsA(UsdGeom.Xformable):  # type: ignore[arg-type]
        return None
